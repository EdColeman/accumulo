...
/**
 * Provides a way to block operations until a resource signal that it is ready. This class
...
public class ReadyMonitor {
  ...
  public ReadyMonitor(final String resourceName) {
   ...
   *           if the resouce does not signal ready withing timeout.
   */
  public void isReady(final long timeout) {
  ...
  /**
   * Indicate that resource is NOT available and that calls to {@link #isReady(long)} should block.
   */
  ...
  public String toString() {
    return new StringJoiner(", ", ReadyMonitor.class.getSimpleName() + "[", "]")
        .add("zkIsConnected=" + isReady).toString();
  }
}
