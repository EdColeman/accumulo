...
public class ConfigurationUpgrade {
  ...
  public void convertSystem() throws KeeperException, InterruptedException {
    upgrade(getSrcSysPath(), CacheId.forSystem(context));
  }
  ...
  public void convertNamespaces() throws KeeperException, InterruptedException {
    ...
    for (String ns : namespaces) {
      ...
      upgrade(path, CacheId.forNamespace(context, nid));
    }
  }
  ...
  public void convertTables() throws KeeperException, InterruptedException {
    ...
    for (String t : tables) {
      ...
      log.info("Looking for: {}", path);
      upgrade(path, CacheId.forTable(context, tid));
    }
  }
  ...
  public void upgrade(final String srcPath, final CacheId destId)
    ...
    String destPath = String.format("%s/%s", destBasePath, destId.nodeName());
  ...
  public boolean verifyAll() {
    try {
      ...
      // check system
      convertedCount++;
      if (!verifyById(CacheId.forSystem(context))) {
        return false;
      }
      ...
      for (String ns : namespaces) {
        ...
        var path = getSrcNsPath(nid);
        log.info("Looking for: {}", path);
        if (!verifyById(CacheId.forNamespace(context, nid))) {
          return false;
        }
      }
      ...
      for (String t : tables) {
        ...
        var path = getSrcTablePath(tid);
        log.info("Looking for: {}", path);
        if (!verifyById(CacheId.forTable(context, tid))) {
          return false;
        }
      }
  ...
  public boolean verifyById(final CacheId id) {
  ...
  private boolean verifyNode(final CacheId id, final String srcPath) {
    ...
    try {
      ...
      Stat convertedNode = zooKeeper.exists(id.path(), false);
      ...
      PropEncoding converted =
          new PropEncodingV1(zooKeeper.getData(id.path(), false, convertedNode));
