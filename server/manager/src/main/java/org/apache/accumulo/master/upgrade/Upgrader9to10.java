...
public class Upgrader9to10 implements Upgrader {
  ...
  private void setMetaTableProps(ServerContext ctx) {
    ...
    try {
      ...
      PropCache propCache = ctx.getPropCache();

      var rootId = new CacheId(ctx.getInstanceID(), null, RootTable.ID);
      ...
      propCache.setProperties(rootId, rootProps);

      var metadataId = new CacheId(ctx.getInstanceID(), null, RootTable.ID);
      Map<String,String> metadataProps = new HashMap<>();

      metadataProps.put(Property.TABLE_COMPACTION_DISPATCHER.getKey(),
          SimpleCompactionDispatcher.class.getName());

      metadataProps.put(Property.TABLE_COMPACTION_DISPATCHER_OPTS.getKey() + "service", "meta");

      propCache.setProperties(metadataId, rootProps);
  ...
  private void renameOldMasterPropsinZK(ServerContext ctx) {
    ...
    var zooConfiguration = new SystemConfiguration(ctx, new ConfigurationCopy());
